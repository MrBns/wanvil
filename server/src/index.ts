...
// ─── Exported Boot Function (called by CLI) ──────────────────────
...
export async function bootServer(config: IServerConfig): Promise<ServerHandle> {
...
async function main(): Promise<void> {
  ...
  const bridge: IAnvilBridge = createDevBridge();
  const presets: IPresetManager = createDevPresetManager();
  ...
  if (anvilArgs.length > 0) {
    ...
    try {
      ...
      for (let i = 0; i < anvilArgs.length; i++) {
        if (anvilArgs[i] === "--fork-url") forkUrl = anvilArgs[i + 1] || "";
        if (anvilArgs[i] === "--fork-block-number") forkBlockNumber = anvilArgs[i + 1] || "";
        if (anvilArgs[i] === "--mnemonic") mnemonic = anvilArgs[i + 1] || "";
      }
...
import type { IMnemonicPreset } from "../../shared/dist/bridge.js";

function createDevBridge(): IAnvilBridge {
  ...
  return {
    start(args: string[]): void {
      ...
      child.on("close", (code, signal) => {
        if (lineBuffer) { pushLine(lineBuffer); lineBuffer = ""; }
        proc = null;
    ...
    getLogSnapshot: () => [...logs],
    onLog(cb: (line: string) => void) { events.on("log", cb); return () => events.off("log", cb); },
    onClose(cb: (code: number | null, signal: string | null) => void) {
      events.on("close", cb);
...
function createDevPresetManager(): IPresetManager {
  return {
    async list(): Promise<IMnemonicPreset[]> { return []; },
    async save(_label: string, _mnemonic: string, _password: string): Promise<number> { return 0; },
    async load(_id: number, _password: string): Promise<string> {
