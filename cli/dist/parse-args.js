 ...
 */
import { DEFAULT_WANVIL_FLAGS, WANVIL_FLAG_DEFS, WANVIL_FLAGS, } from "../../shared/dist/args.js";
/**
...
export function parseArgs(raw) {
    const wanvil = { ...DEFAULT_WANVIL_FLAGS };
    const anvil = [];
    let i = 0;
    while (i < raw.length) {
        const token = raw[i];
        ...
        if (WANVIL_FLAGS.has(flagPart)) {
            const def = WANVIL_FLAG_DEFS.find((d) => d.long === flagPart || d.short === flagPart);
            if (!def) {
 ...
 */
function applyFlag(flags, flag, value) {
    switch (flag) {
        case "--gui-port":
            if (value)
                flags.guiPort = Number.parseInt(value, 10) || flags.guiPort;
            break;
        case "--no-gui":
            flags.noGui = true;
            break;
        ...
        case "--preset":
            if (value)
                flags.preset = Number.parseInt(value, 10);
            break;
        case "--save-preset":
