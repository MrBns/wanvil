...
export async function boot(): Promise<void> {
  ...
  if (flags.preset !== null) {
    const password = await promptHidden(
      `🔑 Enter password for preset #${flags.preset}: `,
    );
    try {
      const mnemonic = await presets.load(flags.preset, password);
      anvilArgs.push("--mnemonic", mnemonic);
      process.stdout.write(
        `  \x1b[32m✓\x1b[0m Preset #${flags.preset} decrypted and injected.\n\n`,
      );
    } catch (err) {
  ...
  if (!flags.noAnvil) {
    ...
    process.stdout.write(
      `  \x1b[36m●\x1b[0m --no-anvil mode: skipping anvil spawn (assuming anvil is running externally)\n\n`,
    );
  }
  ...
  if (!flags.noGui) {
    try {
      ...
      process.stderr.write(
        `\x1b[31m✗ Failed to start GUI server: ${err instanceof Error ? err.message : String(err)}\x1b[0m\n`,
      );
...
function getAnvilAvailabilityError(): string | null {
  try {
...
async function loadBootServer(): Promise<typeof import("../../server/src/index.js")> {
  try {
...
async function savePresetFlow(presets: PresetManager): Promise<void> {
  ...
  try {
    ...
    process.stdout.write(
      `  Use it with: \x1b[1mwanvil --preset=${id}\x1b[0m\n\n`,
    );
  } finally {
    rl.close();
  }
